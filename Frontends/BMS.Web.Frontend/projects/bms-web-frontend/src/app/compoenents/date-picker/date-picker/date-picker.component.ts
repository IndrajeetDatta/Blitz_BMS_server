...
export class DatePickerComponent implements OnInit {
  ...
  @Output() datepickerEmitOnChange: EventEmitter<string> =
  ...
  clickedOut() {
    if (this.openTimePicker) this.openTimePicker = false;
  }

  ngOnInit(): void {
    if (this.setMaxDate) {
      if (typeof this.setMaxDate === 'boolean') this.setMaxDate = new Date();

      this.maxDateModel = {
        year: this.setMaxDate.getFullYear(),
        month: this.setMaxDate.getMonth() + 1,
        day: this.setMaxDate.getDate(),
      };
    }
  ...
  displayDate(): string {
    ...
    else formDate = formatDate(date, 'YYYY-MM-dd', 'en-US');
