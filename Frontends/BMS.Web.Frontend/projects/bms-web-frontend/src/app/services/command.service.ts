...
export class CommandService {
  ...
  disableTypeCommandButtonsList: {
    commandType?: string;
    specificIdentify?: any;
  }[] = [];
  ...
  private readonly RestartAppListToString: string[] = this.RestartAppList.map(
  ...
  async postCommand(
    ...
    payload?: string,
    type?: BMSWebApiClientModule.CommandType,
    additionalValue?: any
  ) {
    ...
    try {
      const noOfPendingCommands = await this.chargeStationService.postCommand(
        command
      );

      if (noOfPendingCommands >= 5)
        this.setDisableCommandButtons(
          true,
          'There are more than 5 commands in pending for this charge controller.'
        );
      if (command.type && this.RestartAppList.includes(command.type))
        this.setDisableCommandButtons(
          true,
          'There are REBOOT commands in pending for this charge controller.'
        );
  ...
  async getPendingOrProcessedCommands(
    masterId?: string
  ): Promise<BMSWebApiClientModule.CommandHistory[]> {
    try {
      if (!masterId) throw 'No master id';

      const commands = await this.chargeStationService.getCommands(
        masterId,
        true
      );
      ...
      if (commands.length >= 5)
        this.setDisableCommandButtons(
          true,
          'There are more than 5 commands in pending / processing for this charge controller.'
        );
      else if (
        commands.find(
          (command) =>
            command.commandType &&
            this.RestartAppListToString.includes(command.commandType)
        )
      ) {
        this.setDisableCommandButtons(
          true,
          'There are REBOOT commands in pending / processing for this charge controller.'
        );
      } else this.setDisableCommandButtons(false);

      this.disableTypeCommandButtonsList = [];
      commands.forEach((command) => {
        this.addDisableTypeCommandButton(command);
      });
  ...
  private addDisableTypeCommandButton(
    command:
      | BMSWebApiClientModule.Command
      | BMSWebApiClientModule.CommandHistory,
    additionalValue?: any
  ) {
    ...
    if (command instanceof BMSWebApiClientModule.Command) {
      ...
      specificIdentify = command.type
        ? [
            this.commandType.EnableDisableChargePoint,
            this.commandType.SaveChargingPoint,
          ].includes(command.type)
          ? additionalValue
          : [this.commandType.EditRFID, this.commandType.DeleteRFID].includes(
              command.type
            )
          ? additionalValue
          : undefined
      ...
      specificIdentify = command.commandType
        ? [
            this.commandType.EnableDisableChargePoint.toString(),
            this.commandType.SaveChargingPoint.toString(),
          ].includes(command.commandType)
          ? command.chargePointUid
          : [
              this.commandType.EditRFID.toString(),
              this.commandType.DeleteRFID.toString(),
            ].includes(command.commandType)
          ? command.rfidSerialNumber
          : undefined
        : undefined;
    }
  ...
  private setDisableCommandButtons(
  ...
  isDisabledCommand(commandType?: string, specificIdentify?: any) {
    return !!this.disableTypeCommandButtonsList.find(
      (x) =>
        x.commandType === commandType && x.specificIdentify === specificIdentify
    );
  }
  ...
  tooltipMessage(commandType?: string, specificIdentify?: any) {
    return this.disableCommandButtons
      ...
      : commandType && this.isDisabledCommand(commandType, specificIdentify)
      ? 'This type of command already exists in pending / processing commands.'
      : '';
  }
}
