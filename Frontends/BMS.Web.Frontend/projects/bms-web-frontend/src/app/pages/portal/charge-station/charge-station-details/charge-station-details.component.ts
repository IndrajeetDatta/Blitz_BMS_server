...
export class ChargeStationDetailsComponent implements OnInit {
  ...
  directionSorting: {
    [key in keyof BMSWebApiClientModule.ChargePoint]?: directionSort;
  } = {
    serialNumber: '',
    chargeControllerUid: '',
    name: '',
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      this.installers =
        await this.chargeStationService.getInstallersForChargeStation(this.id);
      if (
        this.chargeController.installer != undefined &&
        this.chargeController.installer != null
      ) {
  ...
  async rebootCommand(): Promise<void> {
    try {
      await this.commandService.postCommand(
        ...
        undefined,
        undefined,
        BMSWebApiClientModule.CommandType.RestartApp
      );
      this.snackBar.open(
  ...
  async restartApp(
    ...
    if (this.commandService.disableCommandButtons) return;
    try {
      await this.commandService.postCommand(
        this.chargeController.id,
        undefined,
  ...
  async toggleChargePoint(chargePointIndex: number): Promise<void> {
    try {
      const chargePoint = new BMSWebApiClientModule.ChargePoint(
        this.stationsPointData[chargePointIndex]
      );
      ...
      await this.commandService.postCommand(
        undefined,
        chargePoint.id,
        JSON.stringify(chargePoint),
        BMSWebApiClientModule.CommandType.EnableDisableChargePoint,
  ...
  async changeTestMode() {
    if (this.chargeController.allowTestModeCommands)
      this.chargeController.allowTestModeCommands = false;
    else this.chargeController.allowTestModeCommands = true;

    try {
      const response = await this.chargeStationService.updateConfiguration(
        this.chargeController,
        this.chargeController.id!
      );

      this.snackBar.open(
        this.translate.instant('general.change-submitted'),
        this.translate.instant('general.ok'),
        { duration: 3000 }
      );
    } catch (e) {
  ...
  editChargePoint(id?: number) {
    this.router.navigate(['/portal/charge-station/charge-point-edit'], {
      queryParams: { id },
    });
  }
  ...
  getLoadManagement(id?: number) {
    this.router.navigate(['/portal/charge-station/load-management'], {
      queryParams: { id },
    });
  }

  getWhiteList(id?: number) {
    this.router.navigate(['/portal/charge-station/whitelist'], {
      queryParams: { id },
    });
  }

  getOCCP(id?: number) {
    this.router.navigate(['/portal/charge-station/occp'], {
      queryParams: { id },
    });
  }

  getUserData(id?: number) {
    this.router.navigate(['/portal/charge-station/user-data'], {
      queryParams: { id },
    });
  }
  ...
  getCommandHistory(masterId?: string, id?: number) {
    this.router.navigate(['/portal/charge-station/commands-history'], {
      queryParams: { masterId, id },
    });
  }

  getPortSharing(id?: number) {
    this.router.navigate(['/portal/charge-station/port-sharing'], {
      queryParams: { id },
    });
  }
  ...
  getLogs(id?: number) {
    this.router.navigate(['/portal/charge-station/logs'], {
      queryParams: { id },
    });
  }

  getEmails(id?: number) {
    this.router.navigate(['/portal/charge-station/emails'], {
      queryParams: { id },
    });
  }

  showAllInstallers(id?: number) {
    this.installersDialog.open(UserAccessDialogComponent, {
      data: this.installers,
    });
  }
  ...
  getStatusBgColor(state?: string) {
    if (state === 'A1') return 'green';
    else if (state === 'C2') return 'cyan';
    else if (state === 'E') return 'red';
    else if (state === 'B2') return 'orange';
  ...
  async setLastMaintenance(): Promise<void> {
    try {
      ...
      const newChargeController =
        await this.chargeStationService.updateConfiguration(
          this.chargeController,
          this.chargeController.id!
        );
      if (newChargeController.id)
        this.chargeController.lastMaintenance =
          newChargeController.lastMaintenance;
      else throw 'Could not save';

      this.snackBar.open(
        this.translate.instant(
          'charge-station.details.last-maintenance-success-set'
        ),
        this.translate.instant('general.ok'),
        { duration: 3000 }
      );
